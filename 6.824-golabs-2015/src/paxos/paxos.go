...
func (px *Paxos) Prepare(args *PrepareArgs, reply *PrepareReply) error {
	...
	fmt.Println("Prepare: %d, %d, %s", px.me, args.Seq, args.PNum);

	//then check the Seq
	inst, exist := px.instances[args.Seq]
	if !exist {
	...
	//set the reply
	if reply.Err == OK {
		reply.AcceptPnum = inst.n_a
		reply.AcceptValue = inst.v_a
...
func (px *Paxos) Accept(args *AcceptArgs, reply *AcceptReply) error {
	...
	fmt.Println("Accept: %d, %d, %s", px.me, args.Seq, args.PNum);
...
func (px *Paxos) propose(seq int, v interface{}) {
	...
	for {
		...
		preargs := PrepareArgs{Seq: seq, PNum: pnum}
		replyNum := 0
		replyMEpoch := ""
		replyValue := v
		for i, peer := range px.peers {
			var reply PrepareReply
			fmt.Println("sendPrepare: %d, %d, %s", px.me, preargs.Seq, preargs.PNum)
			if i == px.me {
				px.Prepare(&preargs, &reply)
			} else {
				call(peer, "Paxos.Prepare", &preargs, &reply)
			}

			if reply.Err == OK {
				replyNum += 1
				if reply.AcceptPnum > replyMEpoch {
					replyMEpoch = reply.AcceptPnum
		...
		accNum := 0
		if replyNum >= px.majority() {
			accargs := AcceptArgs{Seq: seq, PNum: pnum, Value: replyValue}
			for i, peer := range px.peers {
				fmt.Println("sendAccept: %d, %d, %s", px.me, accargs.Seq, accargs.PNum)
				var accreply AcceptReply
				if i == px.me {
					px.Accept(&accargs, &accreply)
				} else {
					call(peer, "Paxos.Accept", &accargs, &accreply)
				}
		...
		//tell other peers the dicided value
		if accNum >= px.majority() {
			decargs := DecideArgs{Seq: seq, Value: replyValue, PNum: pnum, 
